...
For non-observable sources (`List<T>`), you must rebind the `ItemsSource` after a change. Source: `articles/controls-and-libraries/tree-list.md` references `Add and Remove Nodes` at `xref:6123`.
...
`TreeListControl` supports Edit Forms (a popup form for editing a row instead of inline editing). See `articles/controls-and-libraries/data-grid/data-editing-and-validation/` (shared with GridControl) and `xref:403491`.
...
The cell- and row-validation pipeline is shared with GridControl. See `xref:7358` in the docs.
