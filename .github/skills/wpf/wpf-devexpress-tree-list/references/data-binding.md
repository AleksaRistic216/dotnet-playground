...
For trees backed by remote services or DB queries, fetch children on demand so the UI stays responsive. The article (`xref:10366#fetch-nodes-asynchronously`) covers this pattern. The interface is `IAsyncChildNodesSelector` (or similar — verify):
